...
export function StepDestination({ form }: StepDestinationProps) {
    ...
    return (
        <div className="flex flex-col gap-3">
            <div>
                <h3 className="text-lg font-semibold text-primary">Where do you want to go?</h3>
                <p className="text-sm text-muted">Pick the city Travix should plan your trip around.</p>
            </div>
            ...
            {formState.errors.cityId && <span className="text-xs text-red-500">{formState.errors.cityId.message}</span>}
        </div>
    )
}

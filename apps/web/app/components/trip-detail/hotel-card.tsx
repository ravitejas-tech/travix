...
export function HotelCard({ hotel }: HotelCardProps) {
    ...
    return (
        ...
        >
            ...
            <span className={`w-fit rounded-full px-2.5 py-0.5 text-xs font-medium ${meta.className}`}>
                {meta.label}
            </span>
            {hotel.description && <p className="text-sm text-muted">{hotel.description}</p>}
        </motion.div>
    )
}

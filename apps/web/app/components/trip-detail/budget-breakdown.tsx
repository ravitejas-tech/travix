...
export function BudgetBreakdown({ budget }: BudgetBreakdownProps) {
    if (!budget) {
        return <p className="py-12 text-center text-sm text-muted">No budget estimate available.</p>
    }
    ...
    return (
        <div className="mx-auto max-w-xl rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
                        ...
                        <div key={row.key} className="flex items-center gap-3">
                            ...
                            <div className="flex-1">
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="font-medium text-primary">{formatMoney(value, symbol)}</span>
                                </div>
            ...
            <div className="mt-5 flex items-center justify-between border-t border-gray-100 pt-4">
                ...
                <span className="text-2xl font-bold text-primary">{formatMoney(budget.total, symbol)}</span>
            </div>
        </div>
    )
}

        ...
        <BentoCard className="flex flex-col justify-center gap-3 bg-white p-5">
            ...
            <div className="flex flex-col gap-2">
                {TRIP_INPUTS.map((field) => {
                    ...
                    return (
                        <div key={field.label} className="flex items-center gap-3 rounded-xl bg-primary/5 px-3 py-1.5">
                            <Icon className="h-4 w-4 text-primary" />
                            <span className="text-xs text-muted">{field.label}</span>
                            <span className="ml-auto text-sm font-medium text-primary">{field.value}</span>
                        </div>
                    )
                })}
            </div>

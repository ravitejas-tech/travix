  ...
  async run(passedParams: string[], options?: Record<string, any>) {
    ...
    try {
      ...
      try {
        ...
        sqlInMemory.upQueries.forEach((q) => {
          upSqls.push(
            `        await queryRunner.query(\`${q.query.replace(/`/g, '\\`')}\`${this.params(q.parameters)});`,
          );
        });
        sqlInMemory.downQueries.forEach((q) => {
          downSqls.push(
...
`;
      ...
      if (dryrun) {
        this.logger.log(
          chalk.green(`Dry run — migration content:\n\n${content}`),
        );
      } else {
        ...
        this.logger.log(
          chalk.green(
            `Migration ${chalk.blue(filename)} generated successfully.`,
          ),
        );
      }
    } catch (err) {

...
import { CreateTripPayload, LimitQuery, PageQuery, TripIdParam } from '../../dtos/payloads'
import { PaginatedTripsResponse, TripDetailResponse } from '../../dtos/responses'
...
export class V1TripsController {
    ...
    async create(
        ...
        return this.commandBus.execute(Builder(CreateTripCommand, { userId, payload: body }).build())
    }
    ...
    async list(
        ...
        return this.queryBus.execute(Builder(GetTripsQuery, { userId, page, limit }).build())
    }
    ...
    async detail(tripId: string, @AuthUser('id') userId: string): Promise<Static<typeof TripDetailResponse>> {
        return this.queryBus.execute(Builder(GetTripQuery, { userId, tripId }).build())
    }
    ...
    async remove(tripId: string, @AuthUser('id') userId: string): Promise<void> {
        await this.commandBus.execute(Builder(DeleteTripCommand, { userId, tripId }).build())
    }
}

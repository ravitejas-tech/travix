...
export const CreateTripPayload = Type.Object({
    ...
    budgetType: LiteralUnion(Object.values(BudgetType) as [BudgetType, ...BudgetType[]], {
        example: BudgetType.MEDIUM,
    }),
    interests: Type.Array(Type.String({ example: 'food' }), { example: ['food', 'culture'] }),
    currencyCode: Type.Optional(Type.String({ minLength: 3, maxLength: 3, default: 'USD', example: 'USD' })),
})

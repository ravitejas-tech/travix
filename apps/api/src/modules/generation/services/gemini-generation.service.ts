...
export class GeminiGenerationService extends GenerationService {
    ...
    async generateHotels(context: GenerationContext): Promise<GeneratedHotel[]> {
        ...
        const result = await this.generate<{ hotels: GeneratedHotel[] }>(prompt, generationHotelsSchema)
    ...
    private async generate<T>(prompt: string, responseSchema: object): Promise<T> {
        try {
            ...
            this.logger.error('Gemini generation failed', error instanceof Error ? error.stack : String(error))

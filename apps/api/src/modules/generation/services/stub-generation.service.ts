...
export class StubGenerationService extends GenerationService {
    ...
    async generateTrip(context: GenerationContext): Promise<GeneratedTrip> {
        ...
        const days: GeneratedDay[] = Array.from({ length: context.numberOfDays }, (_, index) =>
            this.buildDay(context, index + 1),
        )
    ...
    private buildDay(context: GenerationContext, dayNumber: number): GeneratedDay {
        const interests = context.interests.length > 0 ? context.interests : ['sightseeing']
        const activities: GeneratedActivity[] = interests.slice(0, 3).map((interest) => ({
            ...
            type: INTEREST_TO_ACTIVITY[interest.toLowerCase()] ?? ActivityType.OTHER,
        }))
    ...
    private buildHotels(context: GenerationContext): GeneratedHotel[] {
        const categories: HotelCategory[] = [HotelCategory.BUDGET, HotelCategory.MID_RANGE, HotelCategory.LUXURY]
        return categories.map((category) => ({
            name: `${context.destination} ${this.capitalize(category.replace('_', ' '))} Stay`,
            category,
            rating: category === HotelCategory.LUXURY ? 4.8 : category === HotelCategory.MID_RANGE ? 4.2 : 3.7,

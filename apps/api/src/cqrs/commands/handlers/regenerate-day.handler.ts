...
import { ActivityEntity, BudgetEstimationEntity, ItineraryDayEntity, TripEntity } from '@travix/db'
...
export class RegenerateDayHandler implements ICommandHandler<RegenerateDayCommand> {
    ...
    async execute(command: RegenerateDayCommand): Promise<Static<typeof ItineraryDayResponse>> {
        ...
        const activities = await manager.transaction(async (trx) => {
            const merged = await this.replaceGeneratedActivities(trx, day, generated.activities)
    ...
    private async replaceGeneratedActivities(
        ...
        generated: { name: string; description: string | null; type: ActivityEntity['type'] }[],
    ): Promise<ActivityEntity[]> {
        await manager.softDelete(ActivityEntity, { itineraryDayId: day.id, isCustom: false })

        const custom = await manager.find(ActivityEntity, { where: { itineraryDayId: day.id, isCustom: true } })

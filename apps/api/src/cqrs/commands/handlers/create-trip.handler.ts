...
export class CreateTripHandler implements ICommandHandler<CreateTripCommand> {
    ...
    async execute(command: CreateTripCommand) {
        ...
        const generated = await this.generationService.generateTrip({
            destination: city.country ? `${city.name}, ${city.country.name}` : city.name,
    ...
    private async resolveCity(manager: EntityManager, cityId: string): Promise<CityEntity> {
        const city = await manager.findOne(CityEntity, { where: { id: cityId }, relations: ['country'] })
        if (!city) {
    ...
    private async resolveCurrency(manager: EntityManager, code: string): Promise<CurrencyEntity> {

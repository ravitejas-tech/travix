...
import { StaticDecode, TypeGuard, Type } from '@sinclair/typebox'
...
import { TypeboxTransformInterceptor } from './interceptors'
import type {
...
>(validatorConfig: ValidatorConfig<T, RequestValidators>): MethodDecorator<MethodDecoratorType> {
    return (target, key, descriptor) => {
        ...
        const methodName = `${target.constructor.name}${capitalize(String(key))}`
        ...
        requestValidatorConfigs?.forEach((validatorConfig, index) => {
            switch (validatorConfig.type) {
                case 'body': {
                    ...
                    const validatorPipe: PipeTransform = {
...
>(
    ...
    const { method, responseCode = 200, path, validate, auth, ...apiOperationOptions } = config

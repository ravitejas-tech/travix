import type { TArray, TImport, TIntersect, TObject, TRecord, TRef, TSchema, TTuple, TUnion } from '@sinclair/typebox'
...
function FromImport(schema: TImport, analysis: SchemaAnalysis) {
    Object.values(schema.$defs).forEach((s: any) => Visit(s, analysis))
}
...
function Visit(schema: TSchema, analysis: SchemaAnalysis): void {
    ...
    switch (schema[Kind]) {
        ...
        case 'Import':
            return FromImport(schema as TSchema as TImport, analysis)
    }
}
